    ...
    for i, data in enumerate(distr_dl):
        ...
        if i % 100 == 0:
            if distr_backend.is_root_worker():
                ...
                ecg_plot.save_as_png("Hard_Reconstructed_ECG2")



                logs = {
                    **logs,
                    'sample images':        wandb.log({"Original ECG": wandb.Image("Original_ECG2.png")}),
                    'reconstructions':      wandb.log({"Reconstructed ECG": wandb.Image("Reconstructed_ECG2.png")}),
                    'hard reconstructions': wandb.log({"Hard Reconstructed ECG": wandb.Image("Hard_Reconstructed_ECG2.png")}),
                    'codebook_indices':     wandb.Histogram(codes.cpu()),
                    'temperature':          temp
                }

                wandb.save('./vae/vae2.pt')
            save_model(f'./vae/vae2.pt')
